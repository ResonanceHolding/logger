...
class Logger {
  ...
  init({
    ...
    if (sentryConfig) {
      if (!sentryConfig.environment)
        throw new Error('Sentry environment must be specified');
   ...
   * to Sentry.
   *
   * If throttling enabled - skips messages that exceeds throttling limits.
   * Every skipped error will be counted and its amount will be displayed
  ...
  debug(...args) {
    if (this.isSentry) {
      Sentry.captureMessage(JSON.stringify(args, null, 2));
    }
  ...
  isThrottledError(e) {
    ...
    if ([this.#errors.keys()].length > this.errorsLength) {
      // if errors map is bigger than size limit
      let oldestError = [...this.#errors.keys()][0];
      this.#errors.forEach((value, key) => {
        if (value < this.#errors.get(oldestError)) oldestError = key;
      });
